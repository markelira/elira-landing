  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      if (errorCode === 'auth/wrong-password' || errorCode === 'auth/invalid-credential') {
        setError('Helytelen jelszó. Kérlek ellenőrizd és próbáld újra.');
      } else if (errorCode === 'auth/user-not-found') {
        setError('Nincs fiók ezzel az email címmel. Szeretnél regisztrálni?');
      } else if (errorCode === 'auth/invalid-email') {
        setError('Érvénytelen email cím formátum.');
      } else if (errorCode === 'auth/user-disabled') {
        setError('Ez a fiók le van tiltva. Kérlek vedd fel velünk a kapcsolatot.');
      } else if (errorCode === 'auth/too-many-requests') {
        setError('Túl sok sikertelen próbálkozás. Kérlek próbáld újra később.');
      } else {
        setError(err.message || 'Bejelentkezés sikertelen. Kérlek próbáld újra.');
      }
    } finally {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (errorCode === 'auth/popup-closed-by-user') {
        setError('Bejelentkezési ablak bezárva. Kérlek próbáld újra.');
      } else if (errorCode === 'auth/popup-blocked') {
        setError('A böngésző blokkolta a bejelentkezési ablakot. Kérlek engedélyezd a felugró ablakokat.');
      } else if (errorCode === 'auth/account-exists-with-different-credential') {
        setError('Ez az email cím már regisztrálva van más bejelentkezési módszerrel.');
      } else {
        setError(err.message || 'Google bejelentkezés sikertelen. Kérlek próbáld újra.');
      }
    } finally {
      setIsSubmitting(false);
    }
